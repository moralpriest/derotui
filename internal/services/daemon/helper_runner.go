...
func (s *helperState) status() (Snapshot, wallet.DaemonInfo, []string) {
	...
	info := wallet.DaemonInfo{
		...
		IsOnline:              true,
		IsHealthy:             true,
		IsSynced:              peerHeight > 0 && height > 0 && height+2 >= peerHeight,
		IsSyncing:             peerHeight > 0 && height > 0 && height+2 < peerHeight,
		IsBootstrapping:       (height <= 0 && peerHeight > 0) || (peerHeight > 0 && height > 0 && topoHeight != height && peerHeight-height > 50) || p2p.IsBootstrapActive(),
		IsFinalizingBootstrap: peerHeight > 0 && height > 0 && topoHeight > 0 && topoHeight < height && height+2 >= peerHeight,
